...
**Next skill to add: `handoff`.** Most of its contract is deterministic (section names/order, ≤5 bullets, ~150 words, closing imperative, no report-shape headings). Two catches: its fixture is a session transcript (the priciest kind), and its workflow is interactive — headless runs truncate at the "good to copy?" prompt, so assert only up to the print-and-ask.
